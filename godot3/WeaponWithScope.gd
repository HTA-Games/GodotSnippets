...
export(FiringMode) var primary_fire_mode := FiringMode.Automatic
...
export(OverflowMode) var primary_overflow_mode := OverflowMode.FillFromReserve


export(FiringMode) var secondary_fire_mode := FiringMode.Automatic
...
export(OverflowMode) var secondary_overflow_mode := OverflowMode.FillFromReserve
...
export(ReloadMode) var reload_mode := ReloadMode.FullClip
		...
		FiringMode.ChargeAndRelease:
			...
			if (overcharge_time < 0.0 or charge_time < max_charge + overcharge_time) and \
				...
				if charge_time < max_charge:
					required_ammo = int(range_lerp(charge_time, 0.0, max_charge, min(ammo_cost, 1), ammo_cost)) - charge_ammo_consumed
				else:
				...
				emit_signal(str("charged_", fire_signal),
					clamp(range_lerp(charge_time, min_charge, max_charge, 0.0, 1.0), 0.0, 1.0),
