...
const MIN_CAMERA_ANGLE := deg2rad(-85.0)
const MAX_CAMERA_ANGLE := deg2rad(85.0)
...
const gravity :float = ProjectSettings["physics/3d/default_gravity"]
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		...
		var mscale :float = deg2rad(get_node(camera_path).fov) / -res.y
